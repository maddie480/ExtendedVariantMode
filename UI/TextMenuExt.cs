...
namespace ExtendedVariants.UI {
    ...
    public class TextMenuExt {
        public class Slider : TextMenuOptionExt<int> {
            public Slider(string label, Func<int, string> values, int min, int max, int value, int defaultIndex, int mapDefinedIndex) : base(label, defaultIndex, mapDefinedIndex) {
        ...
        public class OnOff : TextMenuOptionExt<bool> {
            public OnOff(string label, bool on, bool defaultValue, bool mapDefinedValue) : base(label, defaultValue ? 1 : 0, mapDefinedValue ? 1 : 0) {
